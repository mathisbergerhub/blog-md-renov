...
function attachBriefPageLinks(items) {
  const articles = items.filter((item) => item.group === "articles" && item.htmlPath);
  for (const item of items) {
    if (item.group !== "briefs" || item.htmlPath) continue;

    const words = Array.from(new Set(searchWords(`${item.title} ${item.category}`)));
    if (!words.length) continue;

    let bestArticle = null;
    let bestScore = 0;
    for (const article of articles) {
      const haystack = searchWords(`${article.title} ${article.category} ${article.filePath} ${article.htmlPath}`).join(" ");
    ...
    if (bestArticle && bestScore >= 1) {
      item.htmlPath = bestArticle.htmlPath;
      item.relatedArticlePath = bestArticle.filePath;
    }
  }
  return items;
}
