...
function yamlList(values = []) {
  const list = values.map((value) => `  - ${yamlString(value)}`).join("\n");
...
function parseGeneratedArticle(text) {
...
function estimateReadingTime(markdown = "") {
  const words = markdown.replace(/```[\s\S]*?```/g, " ").replace(/[#>*_`|~-]/g, " ").replace(/\s+/g, " ").trim().split(/\s+/).filter(Boolean).length;
  return `${Math.max(4, Math.ceil(words / 220))} min`;
}

function buildMarkdown(article, brief) {
  const today = new Date().toISOString().slice(0, 10);
  const tags = Array.from(new Set([article.category_label, brief.product, brief.location, brief.main_keyword, ...(Array.isArray(article.tags) ? article.tags : [])].filter(Boolean))).slice(0, 10);
  return `---
...
- Objectif : article utile, lisible, prêt à être relu puis publié.` }] }],
  ...
  if (!response.ok) throw new Error(data.error?.message || "Erreur OpenAI pendant la génération.");
  return parseGeneratedArticle(extractOutputText(data));
}
...
async function githubPathExists(repository, branch, token, filePath) {
  ...
  const response = await fetch(url, { headers: { Authorization: `Bearer ${token}`, Accept: "application/vnd.github+json", "X-GitHub-Api-Version": "2022-11-28", "User-Agent": "mdrenov-blog-admin" } });
  if (response.status === 404) return false;
  if (!response.ok) {
    const data = await response.json().catch(() => ({}));
    throw new Error(data.message || "Impossible de vérifier le fichier GitHub.");
  }
  return true;
}
...
async function createGithubFile({ repository, branch, token, filePath, content }) {
  ...
  const response = await fetch(url, { method: "PUT", headers: { Authorization: `Bearer ${token}`, Accept: "application/vnd.github+json", "Content-Type": "application/json", "X-GitHub-Api-Version": "2022-11-28", "User-Agent": "mdrenov-blog-admin" }, body: JSON.stringify({ message: `Create AI draft: ${filePath}`, branch, content: Buffer.from(content, "utf8").toString("base64") }) });
  const data = await response.json();
  if (!response.ok) throw new Error(data.message || "Impossible de créer le fichier GitHub.");
  return data;
}
...
async function uniqueArticlePath(repository, branch, token, title) {
  ...
  while (await githubPathExists(repository, branch, token, candidate)) {
    candidate = `content/articles/${base}-${index}.md`;
    index += 1;
  }
  return candidate;
}
...
module.exports = async function formatArticle(req, res) {
  ...
  try {
    ...
    if (!brief.title || !brief.category || !brief.main_keyword) {
      sendJson(res, 400, { error: "Titre, catégorie et mot-clé principal sont obligatoires." });
      return;
    }
    ...
    sendJson(res, 200, { ok: true, status: "draft", filePath, title: article.title, seo_title: article.seo_title, description: article.description, markdown, githubUrl: github.content?.html_url || null, message: "Article généré en brouillon. Relis-le dans Decap avant publication." });
  } catch (error) {
