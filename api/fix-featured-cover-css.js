...
const RULES = `
/* Home featured image: full-bleed cover inside rounded card. */
.mdr-home-page .mdr-home-featured > .mdr-home-media--featured.mdr-media--image {
  display: block;
  width: 100%;
  height: 100%;
  min-height: 300px;
...
.mdr-home-page .mdr-home-featured > .mdr-home-media--featured.mdr-media--image img {
  display: block;
  width: 100%;
  height: 100%;
  min-width: 100%;
...
async function writeFile(path, token, sha, content) {
  return githubRequest(path, "PUT", token, {
    message: "Force la photo une en plein cadre",
...
function patchCss(content) {
  let next = content.replace(/\/\* Home featured image: full-bleed cover inside rounded card\. \*\/[\s\S]*?\.mdr-home-page \.mdr-home-featured > \.mdr-home-media--featured\.mdr-media--image img \{[\s\S]*?\}\n?/g, "");
  next = `${next.trimEnd()}\n\n${RULES}\n`;
  return next;
}
...
module.exports = async function handler(req, res) {
  try {
    ...
    const changed = [];
    for (const path of ["styles.css", "home-editorial.css"]) {
      const file = await readFile(path, token);
      const next = patchCss(file.content);
